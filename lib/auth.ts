            ...
            config: [
                {
                    ...
                    getUserInfo: async (tokens) => {
                        const authResponse = await fetch("https://auth.hackclub.com/api/v1/me", {
                            headers: {
                        ...
                        const cachetResponse = await fetch(`${CACHET_HOST}/users/${userInfo.slack_id}`);

                        if (!cachetResponse.ok) {
                            console.error("Failed to fetch Cachet user info:", cachetResponse.statusText);
                            return null;
                        }
